...
export function ProtectedRoute({ children }: ProtectedRouteProps) {
  ...
  const { data: empresasData, isLoading: isLoadingEmpresas } = useQuery({
    ...
    queryFn: () => empresasService.listar({ idproprietario: user?.id, limit: 1 }),
  ...
  useEffect(() => {
    ...
    if (empresasData.data.length > 0) {
      ...
      if (pathname !== "/empresas/nova" && pathname !== "/assinatura" && !localStorageEmpresa) {
        router.push("/empresas/nova");
      }
    }
  }, [empresasData, localStorageEmpresa, selecionarEmpresa, router, pathname, isMounted, isLoadingEmpresas]);
  ...
  useEffect(() => {
    ...
    if (user && (user.plano === null || user.plano === undefined)) {
      ...
      if (temEmpresa && pathname !== "/assinatura" && pathname !== "/empresas/nova") {
        router.push("/assinatura");
      }
    }
  }, [isAuthenticated, isLoading, user, router, pathname, isMounted, localStorageEmpresa, empresasData]);
  ...
  const isResolvendo =
    !isMounted || isLoading || (isAuthenticated && !localStorageEmpresa && isLoadingEmpresas);
  ...
  if (ehProprietario && user && (user.plano === null || user.plano === undefined) && temEmpresa) {
