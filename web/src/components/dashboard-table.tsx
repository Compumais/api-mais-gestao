...
import { type UltimaMovimentacao, type UltimasMovimentacoes } from "@/services/dashboard.service"
...
function TransactionTable({
    ...
    if (isLoading) {
        return <div className="p-8 text-center text-muted-foreground">Carregando movimentações...</div>
    }

    if (data.length === 0) {
        return <div className="p-8 text-center text-muted-foreground">Nenhuma movimentação encontrada.</div>
    }
                ...
                <TableBody>
                    {data.map((item) => (
                        <TableRow key={item.id}>
                            ...
                            <TableCell className={`text-right font-medium ${item.natureza === 'saida' ? 'text-red-500' :
                                    item.natureza === 'entrada' ? 'text-green-500' : ''
                                }`}>
...
function StatusBadge({ status, type }: { status: string, type: string }) {
    ...
    if (status === 'Pago' || status === 'Recebido' || status === 'Quitado' || status === 'Conciliado') {
