// Lista de bancos padrão que são criados automaticamente quando uma empresa é criada
// Estes bancos não podem ser editados ou excluídos pelo usuário
...
/**
 * Verifica se um banco é um banco padrão (criado automaticamente)
 * @param banco - O banco a ser verificado
 * @returns true se o banco é padrão, false caso contrário
 */
export function isBancoPadrao(banco: {
	codigo: string;
	nome: string;
}): boolean {
	return BANCOS_PADRAO.some(
		(bp) => bp.codigo === banco.codigo && bp.nome === banco.nome,
	);
}
