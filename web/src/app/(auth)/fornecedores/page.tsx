...
function montarParamsFiltroFornecedores(params: {
	idempresa: string;
	qAplicado: string;
	filtrosColuna: FiltrosColunaFornecedoresState;
...
export default function FornecedoresPage() {
	...
	const { data, isLoading, isError, error } = useQuery({
		...
		queryFn: async () => {
			...
			return await entidadesService.listar({
				...montarParamsFiltroFornecedores({
	...
	const exportarFornecedoresMutation = useMutation({
		mutationFn: async () => {
			if (!localStorageEmpresa) {
				throw new Error("Empresa não selecionada");
			}
		...
		onSuccess: (blob) => {
			baixarArquivo(blob, "fornecedores.csv");
			...
			<div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
				<div className="flex items-center justify-between gap-2 px-4">
					...
					<div className="flex flex-wrap items-center gap-2">
						<BotaoExportarCsv
							onExportar={() => exportarFornecedoresMutation.mutate()}
							isPending={exportarFornecedoresMutation.isPending}
