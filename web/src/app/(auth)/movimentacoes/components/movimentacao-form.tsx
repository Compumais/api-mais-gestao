		...
		if (open) {
			if (isEdicao && lancamento) {
				...
				reset({
					...
					idplanocontas: lancamento.idplanocontas?.toString(),
					dataconciliacao:
						formatDateForInput(lancamento.dataconciliacao) || undefined,
		...
		if (isEdicao && lancamento) {
			...
			atualizarMutation.mutate(
				{
					...
					dados: {
						...
						idplanocontas: data.idplanocontas
							...
							: undefined,
						dataconciliacao: data.dataconciliacao,
					},
				},
				{
					onSuccess: () => {
			...
			if (data.operacao === "transferencia") {
				...
				criarMutation.mutate(
					{
						idcontacorrente: (data as CriarContaCorrenteLancamentoFormData)
							.idcontacorrente,
						tipo,
						datahora: data.data,
						valor: data.valor,
							...
							<div className="space-y-4">
											...
											>
												...
												<SelectContent>
													{contasCorrentes
														.filter(
															(conta) =>
																conta.id !== idcontacorrenteDestino,
														)
														.map((conta) => (
															<SelectItem key={conta.id} value={conta.id}>
																{conta.descricao || conta.agencia || conta.id}
								...
								<Field data-invalid={!!errors.operacao}>
									...
									<Select
										value={operacao}
										onValueChange={(value) => {
											setValue("operacao", value as "entrada" | "saida" | "transferencia");
									...
									{!temDuasOuMaisContas && (
										<p className="text-sm text-muted-foreground">
											É necessário ter pelo menos 2 contas correntes para realizar
											transferências.
										</p>
									)}
								</Field>
