...
type EditorModeloImpressaoPedidoProps = {
	nome: string;
	descricao: string;
	primario: boolean;
	...
	onLayoutChange: (layout: LayoutModeloImpressaoPedido) => void;
	somenteLeitura?: boolean;
};

export function EditorModeloImpressaoPedido({
	nome,
	descricao,
	primario,
	layout,
...
}: EditorModeloImpressaoPedidoProps) {
	...
	function atualizarBloco(
		id: string,
		patch: Partial<BlocoModeloImpressaoPedido>,
	) {
			...
			<div className="space-y-4">
				<div className="rounded-lg border p-4 space-y-3">
					<div className="space-y-1">
						<Label htmlFor="nome-modelo-pedido">Nome</Label>
						<Input
							id="nome-modelo-pedido"
							value={nome}
							disabled={somenteLeitura}
					...
					<div className="space-y-1">
						<Label htmlFor="desc-modelo-pedido">Descrição</Label>
						<Input
							id="desc-modelo-pedido"
							value={descricao}
							disabled={somenteLeitura}
				...
				{blocoSelecionado && !somenteLeitura && (
					<div className="rounded-lg border p-4 space-y-3">
						...
						{blocoSelecionado.tipo === "dadosPedido" && (
							<div className="space-y-2">
								{CAMPOS_DADOS_PEDIDO.map((campo) => (
									<label
										key={campo.value}
									...
									>
										<Checkbox
											...
											onCheckedChange={() =>
												toggleCampo(
													campo.value,
													CAMPOS_DADOS_PEDIDO.map((c) => c.value),
												)
											}
										/>
										{campo.label}
						...
						{blocoSelecionado.tipo === "cliente" && (
							<div className="space-y-2">
								{CAMPOS_CLIENTE_PEDIDO.map((campo) => (
									<label
										key={campo.value}
										className="flex items-center gap-2 text-sm"
									>
