...
export default function GerarEfdIcmsPage() {
	...
	const gerarMutation = useMutation({
		mutationFn: async () => {
			...
			return gerarArquivoEfdIcms({
				idempresa: empresa.id,
				dataInicio: periodo.dataInicio,
				dataFim: periodo.dataFim,
				finalidade,
				incluirInventario,
				dataInventario: incluirInventario ? periodo.dataFim : undefined,
			});
		},
		onSuccess: (resultado) => {
			setAlertas(resultado.alertas);
			toast.success(
				`Arquivo EFD ICMS/IPI gerado com ${resultado.totalLinhas} linha(s).`,
			);
		...
		onError: (error: Error) => {
			toast.error(error.message || "Erro ao gerar arquivo EFD ICMS/IPI");
		},
	});
	...
	return (
		<PageContainer>
			<div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
				<div className="px-4">
					<h1 className="text-2xl font-bold">Gerar EFD ICMS/IPI</h1>
					<p className="mt-1 text-sm text-muted-foreground">
						Espelho mensal dos documentos fiscais (leiaute 020) para importação
						no PVA. A entrega ao fisco continua pelo Receitanet.
					</p>
				</div>
				...
				<div className="px-4">
					<Card className="max-w-xl">
						<CardHeader>
							...
							<CardDescription>
								A competência é sempre um mês civil. Confira perfil A/B/C na
							...
							<FieldGroup className="gap-4">
								...
								<div className="flex items-center gap-2">
									...
									<FieldLabel htmlFor="incluirInventario">
										Incluir inventário fiscal (bloco H)
									</FieldLabel>
								</div>
					...
					<div className="px-4">
						<Card className="border-amber-500/40">
							<CardHeader>
								...
								<CardDescription>
									Revise os pontos abaixo antes de abrir o arquivo no PVA.
								</CardDescription>
							</CardHeader>
							<CardContent>
