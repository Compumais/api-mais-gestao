...
export type TipoFiltroColunaOs =
	| "nenhum"
	| "texto"
	| "data"
	| "opcoes"
...
export type OpcaoFiltroColunaOs = {
	value: string;
	label: string;
};

export type OrdenacaoColunaOs = "asc" | "desc" | false;

type CabecalhoColunaOsProps = {
	titulo: string;
	colunaId: string;
	ordenacao: OrdenacaoColunaOs;
	onOrdenar: (direcao: "asc" | "desc" | false) => void;
	filtroAtivo: boolean;
	valorFiltro: string;
	onFiltrar: (valor: string) => void;
	tipoFiltro?: TipoFiltroColunaOs;
	opcoes?: OpcaoFiltroColunaOs[];
	placeholderFiltro?: string;
};

export function CabecalhoColunaOs({
	titulo,
	colunaId,
	ordenacao,
	onOrdenar,
