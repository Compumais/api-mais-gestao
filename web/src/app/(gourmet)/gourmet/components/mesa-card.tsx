...
import { parseInstantUtc } from "@/lib/date";
...
export function MesaCard({ conta, totalParcial, qtdItens }: MesaCardProps) {
	...
	const instante = parseInstantUtc(conta.datacriacao);
	const tempoAberto = instante ? dayjs(instante).fromNow() : null;
			...
			<Card className="cursor-pointer transition-colors hover:border-primary/50 hover:bg-accent/30">
				<CardHeader className="pb-2">
					<div className="flex items-center justify-between">
						<CardTitle className="text-xl">Mesa {conta.numeromesa}</CardTitle>
