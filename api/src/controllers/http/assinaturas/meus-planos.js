import { buscarAssinaturaPorEmpresa, atualizarAssinatura } from "@/repositories/assinatura-repositories";
import { AsaasService } from "@/service/asaas/asaas.service";
import { z } from "zod";
export async function getMeuPlanoController(request, reply) {
    ...
    if (!assinatura) {
        return reply.status(404).send({ message: "Nenhuma assinatura encontrada para esta empresa." });
    }
    // Fetch latest status from Asaas to be sure
    try {
        const asaasSubscription = await AsaasService.getSubscription(assinatura.idassinaturaasaas);
...
export async function cancelarAssinaturaController(request, reply) {
    ...
    try {
        await AsaasService.cancelSubscription(assinatura.idassinaturaasaas);
        await atualizarAssinatura(assinatura.id, {
        ...
        return reply.status(500).send({ message: "Erro ao cancelar assinatura.", error: error.message });
    }
}
