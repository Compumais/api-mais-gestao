...
export async function atualizarContaCorrenteLancamento(request, reply) {
    try {
        if (!request.user) {
            return reply.status(httpNaoAutorizado().status).send(httpNaoAutorizado());
        }
        const idusuario = request.user.id;
        const { id } = atualizarContaCorrenteLancamentoParamsSchema.parse(request.params);
        const dadosValidados = atualizarContaCorrenteLancamentoBodySchema.parse(request.body);
...
//# sourceMappingURL=atualizar.js.map