...
import { excluirContaCorrenteLancamentoService } from "@/service/contacorrentelancamento/excluir-conta-corrente-lancamento";
import { httpNaoAutorizado, httpNaoEncontrado } from "@/util/http-util";
const excluirContaCorrenteLancamentoParamsSchema = z.object({
    id: z.string(),
});
export async function excluirContaCorrenteLancamento(request, reply) {
    try {
        if (!request.user) {
            return reply.status(httpNaoAutorizado().status).send(httpNaoAutorizado());
        }
        const idusuario = request.user.id;
        const { id } = excluirContaCorrenteLancamentoParamsSchema.parse(request.params);
