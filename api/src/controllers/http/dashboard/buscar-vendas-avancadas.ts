...
import { buscarVendasAvancadasService } from "@/service/dashboard/buscar-analytics-dashboard.js";
...
export async function buscarVendasAvancadas(
	...
	try {
		...
		const resultado = await buscarVendasAvancadasService({
		...
		return reply.status(500).send({
			error: "Erro ao buscar vendas avançadas",
			code: "VENDAS_AVANCADAS_ERROR",
		});
	}
}
