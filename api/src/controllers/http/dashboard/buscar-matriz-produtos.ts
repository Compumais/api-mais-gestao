...
import { buscarMatrizProdutosService } from "@/service/dashboard/buscar-analytics-dashboard.js";
...
export async function buscarMatrizProdutos(
	...
	try {
		...
		const resultado = await buscarMatrizProdutosService({
		...
		return reply.status(500).send({
			error: "Erro ao buscar matriz de produtos",
			code: "MATRIZ_PRODUTOS_ERROR",
		});
	}
}
