...
export async function atualizarWebhook(request, reply) {
    try {
        if (!request.user) {
        ...
        const usuarioPertenceEmpresa = await verificarUsuarioPertenceEmpresa(request.user.id, params.idempresa);
        ...
        const integracao = configuracao.integracao || { webhooks: [] };
        const webhooks = integracao.webhooks || [];
        const webhookIndex = webhooks.findIndex((w) => w.id === params.webhookId);
        const webhook = webhooks[webhookIndex];
        if (webhookIndex === -1 || !webhook) {
...
//# sourceMappingURL=atualizar-webhook.js.map