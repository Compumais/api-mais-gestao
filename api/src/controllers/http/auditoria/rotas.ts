...
export async function auditoriaRotas(app: FastifyInstance) {
	...
	app.get("/auditoria", {
		...
		schema: {
			...
			description:
				"Lista os logs de auditoria da empresa do usuário autenticado. Retorna uma lista paginada de registros de auditoria.",
			security: [{ bearerAuth: [] }],
			querystring: {
				type: "object",
				required: ["idempresa"],
