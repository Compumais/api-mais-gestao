...
export const TIPO_ORIGEM_FINANCEIRO_ORDEM_SERVICO = 4;
...
export function obterStatusPadraoPorCodigo(
	codigo: OrdemServicoStatusCodigo,
): OrdemServicoStatusPadrao {
	const encontrado = ORDEM_SERVICO_STATUS_PADRAO.find(
		(item) => item.codigo === codigo,
	);
	if (!encontrado) {
		throw new Error(`Status padrão não encontrado: ${codigo}`);
	}
	return encontrado;
}
