	...
	const itensParaInserir: NovoNotaFiscalItem[] = itensMontados.map(
		({ dados: dadosImportacao, codigoCfopEntrada }, index) => {
			...
			return {
				...
				situacaotributariaipi: itemXml.cstipi ?? null,
				basepis: itemXml.basepis ?? null,
				basecofins: itemXml.basecofins ?? null,
				baseicmsst: itemXml.baseicmsst ?? null,
				valoricmsst: itemXml.icmsst ?? null,
				aliquotaicmsst: itemXml.aliquotaicmsst ?? null,
				basefcp: itemXml.basefcp ?? null,
				valorfcp: itemXml.valorfcp ?? null,
				valorfcpst: itemXml.fcpst ?? null,
				cest: itemXml.cest?.replace(/\D/g, "").slice(0, 7) || null,
