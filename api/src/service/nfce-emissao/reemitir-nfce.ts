...
export async function reemitirNfceService({
	...
	const xmlContingencia = nota.arquivoxmlcontingencia?.trim();
	...
	if (ehContingencia) {
		if (!xmlContingencia) {
			return httpBadRequest(
				"NFC-e de contingência sem XML original; revisão manual obrigatória",
			);
		}
		...
		const resultado = await transmitirNfceContingenciaService({
			...
			serie,
			numero,
			motivo: nota.motivocontingencia ?? "Contingência offline NFC-e",
			datacontingencia: `${nota.datacontingencia ?? ""}T${nota.horacontingencia ?? "00:00:00"}`,
		});
		return resultado as HttpResponse<ResultadoEmissaoNfcePdv>;
	}
