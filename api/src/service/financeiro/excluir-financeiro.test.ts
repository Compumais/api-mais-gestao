...
import { excluirFinanceiroService } from "./excluir-financeiro.js";

vi.mock("@/repositories/financeiro-repositories");

describe("excluirFinanceiroService", () => {
	...
	it("deve excluir financeiro existente com sucesso", async () => {
		vi.mocked(financeiroRepository.buscarFinanceiroPorId).mockResolvedValue(
			financeiroMock,
		);
		vi.mocked(financeiroRepository.deletarFinanceiro).mockResolvedValue(
			{} as any,
		);

		const resultado = await excluirFinanceiroService("financeiro-123");
		...
		expect(financeiroRepository.deletarFinanceiro).toHaveBeenCalledTimes(1);
		expect(financeiroRepository.deletarFinanceiro).toHaveBeenCalledWith(
			"financeiro-123",
		);
	});
	...
	it("deve retornar erro 404 quando financeiro não é encontrado", async () => {
		...
		const resultado = await excluirFinanceiroService("financeiro-inexistente");
		...
		expect(financeiroRepository.deletarFinanceiro).not.toHaveBeenCalled();
	});
	...
	it("deve retornar erro 404 quando financeiro é null", async () => {
		...
		const resultado = await excluirFinanceiroService("financeiro-null");
	...
	it("deve chamar métodos na ordem correta: buscar, excluir", async () => {
		vi.mocked(financeiroRepository.buscarFinanceiroPorId).mockResolvedValue(
			financeiroMock,
		);
		vi.mocked(financeiroRepository.deletarFinanceiro).mockResolvedValue(
		...
		await excluirFinanceiroService("financeiro-123");
