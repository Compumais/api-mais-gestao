import {
	...
	type ReceitasPorCategoriaItem,
} from "@/repositories/relatorios-repositories.js";

export interface GerarRelatorioReceitasPorCategoriaParams {
	idempresa: string;
...
export async function gerarRelatorioReceitasPorCategoria(
	params: GerarRelatorioReceitasPorCategoriaParams,
): Promise<{ content: string; contentType: string; filename: string }> {
	const dados = await buscarReceitasPorCategoria({
...
function gerarRelatorioTXT(
	dados: ReceitasPorCategoriaItem[],
	params: GerarRelatorioReceitasPorCategoriaParams,
	...
	relatorio += "RELATORIO DE RECEITAS POR CATEGORIA\n";
	relatorio += "=".repeat(90) + "\n\n";
	...
	relatorio += "-".repeat(90) + "\n";
	relatorio += "TOTAL GERAL".padEnd(64) + formatCurrency(totalGeral).padStart(26) + "\n";
	relatorio += "=".repeat(90) + "\n";

	return {
		content: relatorio,
		contentType: "text/plain; charset=utf-8",
		filename: `receitas-por-categoria-${params.dataInicio}-${params.dataFim}.txt`,
	};
}
...
<head>
  ...
  <title>Relatório de Receitas por Categoria</title>
  <style>
...
<body>
  ...
  <table>
	...
	return {
		...
		filename: `receitas-por-categoria-${params.dataInicio}-${params.dataFim}.html`,
	};
}
