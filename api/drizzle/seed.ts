...
import { montarPlanoContasPadrao } from "../src/util/plano-contas-padrao.js";
...
async function seed() {
	try {
		...
		const empresas = await db.select().from(schema.empresa);

		for (const empresa of empresas) {
			await db
				.insert(schema.planocontas)
				.values(montarPlanoContasPadrao(empresa.id, timestampMillis));
		}
