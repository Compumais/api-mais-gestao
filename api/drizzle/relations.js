...
import { auditLogs, contacorrente, contacorrentelancamento, contas, empresa, entidade, financeiro, financeirolancamento, motivobaixafinanceiro, planocontas, sessoes, tipodocumentofinanceiro, usuarioEmpresa, usuarios, } from "./schema.js";
...
export const contacorrentelancamentoRelations = relations(contacorrentelancamento, ({ one }) => ({
    contacorrente: one(contacorrente, {
        fields: [contacorrentelancamento.idcontacorrente],
        references: [contacorrente.id],
    }),
}));
export const financeirolancamentoRelations = relations(financeirolancamento, ({ one }) => ({
    financeiro: one(financeiro, {
        fields: [financeirolancamento.idfinanceiro],
        references: [financeiro.id],
    }),
}));
export const tipodocumentofinanceiroRelations = relations(tipodocumentofinanceiro, ({ many, one }) => ({
    financeiros: many(financeiro),
...
export const motivobaixafinanceiroRelations = relations(motivobaixafinanceiro, ({ many }) => ({
