...
import { bigint, boolean, char, date, doublePrecision, foreignKey, index, integer, jsonb, numeric, pgTable, smallint, text, timestamp, uniqueIndex, varchar, } from "drizzle-orm/pg-core";
...
export const usuarios = pgTable("usuarios", {
    ...
    perfil: jsonb("perfil").$type().default(sql `'[]'::jsonb`).notNull(),
...
// Tabelas do sistema
export const empresa = pgTable("empresas", {
    ...
    uniqueIndex("empresas_cnpj_key").using("btree", table.cnpj.asc().nullsLast().op("text_ops")),
    foreignKey({
...
export const usuarioEmpresa = pgTable("usuario_empresas", {
    id: text().primaryKey().notNull(),
...
export const entidade = pgTable("entidade", {
    ...
    index("entidades_idempresa_idx").using("btree", table.idempresa.asc().nullsLast().op("text_ops")),
    index("entidades_email_idx").using("btree", table.email.asc().nullsLast().op("text_ops")),
    foreignKey({
        columns: [table.idempresa],
        foreignColumns: [empresa.id],
...
export const contacorrentelancamento = pgTable("contacorrentelancamento", {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: text().primaryKey().notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    ...
    index("idx_contacorrentelanc_evento").using("btree", table.evento.asc().nullsLast().op("int8_ops")),
]);
export const financeiro = pgTable("financeiro", {
    ...
    index("idx_fin_entid_tipo_status").using("btree", table.identidade.asc().nullsLast().op("int8_ops"), table.tipo.asc().nullsLast().op("bpchar_ops"), table.status.asc().nullsLast().op("bpchar_ops")),
    index("idx_financeiro_emissao").using("btree", table.emissao.asc().nullsLast().op("date_ops")),
    index("idx_financeiro_idfilial").using("btree", table.idempresa.asc().nullsLast().op("text_ops")),
    index("idx_financeiro_sped").using("btree", table.idorigem.asc().nullsLast().op("int8_ops"), table.tipoorigem.asc().nullsLast().op("int2_ops")),
    index("idx_financeiro_status").using("btree", table.status.asc().nullsLast().op("bpchar_ops")),
    index("idx_financeiro_tipo").using("btree", table.tipo.asc().nullsLast().op("bpchar_ops")),
    index("idx_financeiro_vencimento").using("btree", table.vencimento.asc().nullsLast().op("date_ops")),
]);
export const financeirolancamento = pgTable("financeirolancamento", {
    id: text().primaryKey().notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    ...
    index("idx_finlan_evento").using("btree", table.evento.asc().nullsLast().op("int8_ops")),
    index("idx_finlan_idfinanceiro").using("btree", table.idfinanceiro.asc().nullsLast().op("text_ops")),
]);
...
export const clientesAsaas = pgTable("clientes_asaas", {
...
export const configuracoes = pgTable("configuracoes", {
    ...
    notificacoes: jsonb("notificacoes").$type().default(sql `'{}'::jsonb`),
    // Configurações de integração
    integracao: jsonb("integracao").$type().default(sql `'{}'::jsonb`),
    // Configurações de relatórios
    relatorios: jsonb("relatorios").$type().default(sql `'{}'::jsonb`),
    // Configurações de impressão
    impressao: jsonb("impressao").$type().default(sql `'{}'::jsonb`),
    criadoem: timestamp({ precision: 3, mode: "string" })
        .default(sql `CURRENT_TIMESTAMP`)
        .notNull(),
    atualizadoem: timestamp({ precision: 3, mode: "string" }).notNull(),
...
export const centrocusto = pgTable("centrocusto", {
    id: text().primaryKey().notNull(),
...
export const entidadecontacontabil = pgTable("entidadecontacontabil", {
    id: text().primaryKey().notNull(),
...
export const contacontabil = pgTable("contacontabil", {
    id: text().primaryKey().notNull(),
    idcontapai: text(),
    idempresa: text().notNull(),
    inativo: integer().default(0),
...
export const codigoreduzidocontacontabil = pgTable("codigoreduzidocontacontabil", {
    id: text().primaryKey().notNull(),
    idempresa: text().notNull(),
    currenttimemillis: bigint({ mode: "number" }).notNull(),
    datacadastro: date().defaultNow().notNull(),
...
export const integracaocontabilconfiguracao = pgTable("integracaocontabilconfiguracao", {
    id: text().primaryKey().notNull(),
    idempresa: text().notNull(),
    currenttimemillis: bigint({ mode: "number" }).notNull(),
...
export const planocontascontacontabil = pgTable("planocontascontacontabil", {
    id: text().primaryKey().notNull(),
